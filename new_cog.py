...
This file keeps the complete command/UI feature set from old_cog.py while replacing the
Lavalink connection, voice-player connection, search, queueing, and /play startup flow with
the playback logic from bot.py.
"""
...
def _looks_like_track_url(query: str) -> bool:
    return bool(_re.match(r"https?://", (query or "").strip(), flags=_re.IGNORECASE))


def _playlist_resolve_candidates(query: str) -> list[str]:
    query = (query or "").strip()
    if not query:
        return []
    if _looks_like_track_url(query) or query.startswith(("ytsearch:", "ytmsearch:", "scsearch:")):
        return [query]
    return [f"ytmsearch:{query}", f"ytsearch:{query}", query]


def _saved_track_from_playable(track: wavelink.Playable, fallback_title: str) -> dict:
    length_ms = getattr(track, "length", 0) or 0
    try:
        duration = max(0, int(length_ms) // 1000)
    except (TypeError, ValueError):
        duration = 0
    return {
        "title": (getattr(track, "title", None) or fallback_title or "Unknown title"),
        "uploader": (getattr(track, "author", None) or "Unknown"),
        "duration": duration,
        "web_url": (getattr(track, "uri", None) or ""),
    }
...
class PlaylistCreateModal(discord.ui.Modal, title="🎵 Create New Playlist"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        failed = query_cap - len(resolved)
        if not resolved:
            await msg.edit(
                content=(
                    f"❌ I couldn't resolve any of the {query_cap} song(s), so I did not create "
                    f"an empty playlist. Try URLs or more specific `artist - title` lines."
                )
            )
            return
        ...
        if err:
            ...
            return
        added, limit_skip = playlist_manager.add_tracks(interaction.guild.id, pid, resolved, max_tracks=limit)
        skip = limit_skip + failed
...
class PlaylistAddSongsModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if not resolved:
            await status.edit(
                content=(
                    f"❌ I couldn't resolve any of the {query_cap} song(s). "
                    "Try URLs or more specific `artist - title` lines."
                )
            )
            return
        added, limit_skip = playlist_manager.add_tracks(self.guild_id, self.playlist_id, resolved, max_tracks=limit)
        skip = limit_skip + failed
        pl = playlist_manager.get_playlist(self.guild_id, self.playlist_id)
        embed = self.cog._build_playlist_manage_embed(pl, self.playlist_id)
        result_msg = (f"✅ Added **{added}** track(s) to **{self.playlist_name}**."
                    + (f"  {skip} skipped (unresolved or limit reached)." if skip else ""))
...
class PlaylistManageView(discord.ui.View):
    ...
    async def play_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        # Match bot.py's saved-playlist playback flow: enable autoplay, add tracks to the
        # existing queue, and only start playback when the player is idle.
        ...
        try:
            if not player.playing:
                ...
                await player.play(first, volume=30)
            ...
            await interaction.followup.send(f"▶️ Added playlist **{pl['name']}** with **{loaded}** loaded track(s) to the queue.{suffix}")
        except Exception as exc:
            logger.error("[PLAYLIST PLAY] Failed to start playback: %s", exc)
...
class Music(commands.Cog):
    ...
    async def _connect_lavalink_nodes_once(self) -> None:
        """Connect Lavalink nodes once and only mark startup successful when one is ready."""
        if self._nodes_started and self._healthy_connect_nodes():
            return
        async with self._node_init_lock:
            ...
            for idx, cfg in enumerate(LAVALINK_NODES, start=1):
                try:
                    uri = _normalize_lavalink_uri(str(cfg["uri"]))
            ...
            try:
                print(f"🔄 Connecting to Lavalink ({len(nodes)} configured node(s))...")
            ...
            ready_nodes = [
                identifier
                for identifier, node in connected.items()
                if node.status == wavelink.NodeStatus.CONNECTED
            ]
            if not ready_nodes:
                logger.error("Wavelink did not connect to any Lavalink nodes. Check node availability/passwords.")
                self._nodes_started = False
                return

            self._nodes_started = True
            logger.info("Wavelink connected to %d node(s): %s", len(ready_nodes), ready_nodes)
    ...
    def _schedule_idle_hard_disconnect(self, player: wavelink.Player) -> None:
        gid = player.guild.id
        self._cancel_idle_disconnect(gid)

        async def _worker() -> None:
            try:
                await asyncio.sleep(IDLE_HARD_LEAVE_SECONDS)
                latest = self._guild_players.get(gid)
                if latest is not player:
                    return
                if latest.playing or latest.paused or not latest.queue.is_empty:
                    return
                latest._intentional_disconnect = True  # type: ignore[attr-defined]
                self._guild_players.pop(gid, None)
                self._autoplay_history.pop(gid, None)
                await self._safe_disconnect_player(latest, reason="idle hard leave (5m)")
                logger.info("[DISCONNECT] Hard-leave idle player in guild %s after %ss.", gid, IDLE_HARD_LEAVE_SECONDS)
            except asyncio.CancelledError:
                return
            finally:
                self._idle_disconnect_tasks.pop(gid, None)

        self._idle_disconnect_tasks[gid] = asyncio.create_task(_worker())
    ...
    async def _safe_connect(
        ...
        *,
        ...
        """Connect to voice using bot.py playback logic.

        The old cog's node selection/failover factory is intentionally not used here: bot.py simply
        connects the voice channel with ``cls=wavelink.Player`` and waits briefly for the player to
        report connected.
        """
        ...
        if not hasattr(player, "home"):
            player.home = channel  # type: ignore[attr-defined]
        elif player.home != channel:  # type: ignore[attr-defined]
            raise RuntimeError(
                f"You can only play songs in {player.home.mention}, as the player has already started there."  # type: ignore[attr-defined]
            )
    ...
    async def _safe_search(self, query: str) -> Optional[wavelink.Search]:
        """Search exactly like bot.py: force a YouTube search through ``Playable.search``."""
    ...
    async def _resolve_songs(self, queries: list[str], tier: str) -> list[dict]:
        ...
        async def _one(q: str) -> Optional[dict]:
            async with sem:
                q = (q or "").strip()
                for candidate in _playlist_resolve_candidates(q):
                    try:
                        results = await self._safe_search(candidate)
                    except Exception as exc:
                        logger.warning("[PLAYLIST RESOLVE] %r via %r failed: %s", q, candidate, exc)
                        continue
                    if not results:
                        continue
                    if isinstance(results, wavelink.Playlist):
                        playlist_tracks = list(getattr(results, "tracks", []) or [])
                        if not playlist_tracks:
                            continue
                        return _saved_track_from_playable(playlist_tracks[0], q)
                    return _saved_track_from_playable(results[0], q)
                logger.warning("[PLAYLIST RESOLVE] No result for %r", q)
                return None
    ...
    async def play(self, interaction: discord.Interaction, query: str) -> None:
        """Play command with bot.py's exact queue-based playback flow.

        All non-playback features from old_cog.py remain in this cog, but this method intentionally
        mirrors bot.py: defer, get/connect the Wavelink player, force ytsearch, enable autoplay,
        queue playlist/single-track results, and only call player.play() when nothing is playing.
        """
        await interaction.response.defer()
        ...
        try:
            ...
            try:
                player = await self._safe_connect(interaction.guild, channel)  # type: ignore[arg-type]
            ...
            tracks = await self._safe_search(query)
    ...
    async def stop(self, interaction: discord.Interaction) -> None:
        ...
        player: wavelink.Player = interaction.guild.voice_client  # type: ignore[assignment]
        if player is None:
            player = self._guild_players.get(interaction.guild.id)  # type: ignore[assignment]
        if player is None:
            await send_interaction_message(interaction, "❌ The bot is not connected.", ephemeral=True)
            return
        if interaction.user.voice.channel != player.channel:  # type: ignore[union-attr]
    ...
    async def on_wavelink_track_end(self, payload: wavelink.TrackEndEventPayload) -> None:
        ...
        if reason not in ("finished", "stopped"):
            logger.debug("[TRACK END] Ignoring unsupported end reason=%s.", payload.reason)
            return
        ...
        if self._autoplay.get(gid, False) or getattr(player, "autoplay", None) == wavelink.AutoPlayMode.enabled:
            try:
        ...
        logger.info("[IDLE CHECK] Queue empty for guild %s, scheduling hard leave in %ss", gid, IDLE_HARD_LEAVE_SECONDS)
        self._schedule_idle_hard_disconnect(player)
